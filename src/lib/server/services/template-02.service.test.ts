...
// ═══════════════════════════════════════════════════════════════════════════
import { getAllTemplates, getTemplates } from './template-02.service';

// ---------------------------------------------------------------------------
// getAllTemplates
// ---------------------------------------------------------------------------
describe('getAllTemplates()', () => {
	it('returns an array of Template_02 rows', async () => {
		const result = await getAllTemplates();
		expect(Array.isArray(result)).toBe(true);
	});

	it('each row has id, date, code, and desc properties', async () => {
		const result = await getAllTemplates();
